    ...
    <section className="space-y-6 p-4 sm:p-6 lg:p-8 bg-[#0A0A0B] min-h-screen text-[#E1D9F9]">
      ...
      <article className="panel p-6 space-y-4 rounded-2xl border border-[#E1D9F9]/[0.06]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block space-y-1">
            ...
            <input value={targetInput} onChange={(e) => setTargetInput(e.target.value)} type="number" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-sm text-[#E1D9F9]" />
          </label>
          <label className="block space-y-1">
          ...
          <label className="block space-y-1">
            ...
            <input value={briefingInput} onChange={(e) => setBriefingInput(e.target.value)} type="text" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-sm text-[#E1D9F9]" />
          </label>
          <label className="block space-y-1">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <article className="panel p-5 space-y-3 rounded-2xl border border-[#E1D9F9]/[0.06]">
          <h2 className="font-heading text-2xl text-[#E1D9F9]">{editingTaskId ? "✏️ Edit Mission Task" : t("Task Management")}</h2>
          <input value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} placeholder="Task Title" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9] outline-none" />
          <textarea value={newTaskDetail} onChange={(e) => setNewTaskDetail(e.target.value)} placeholder="Task Details" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9] outline-none" rows={3} />
          
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block space-y-1">
              <span className="text-[11px] text-[#E1D9F9]/50">Display Timeline Teks</span>
              <input value={newTaskDeadline} onChange={(e) => setNewTaskDeadline(e.target.value)} placeholder="Day 3 - 20:00" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9]" />
            </label>
            <label className="block space-y-1">
              <span className="text-[11px] text-[#EC5C2A] font-semibold">ISO Gate Close Validator</span>
              <input type="datetime-local" value={newTaskIsoDeadline} onChange={(e) => setNewTaskIsoDeadline(e.target.value)} className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9] font-mono outline-none" />
            </label>
          </div>
          ...
          <div className="mt-4 space-y-2 max-h-48 overflow-y-auto">
            {tasks.map((tk) => (
              <div key={tk.id} className="p-3 bg-black/15 border border-[#E1D9F9]/10 rounded-xl flex justify-between items-center text-xs">
                <span className="font-medium text-[#E1D9F9] truncate max-w-[200px]">{tk.taskId} - {tk.title}</span>
                <div className="flex gap-3">
                  <button onClick={() => { setEditingTaskId(tk.id); setNewTaskTitle(tk.title); setNewTaskDetail(tk.detail); setNewTaskDeadline(tk.deadline); setNewTaskIsoDeadline(tk.isoDeadline || ""); }} className="text-[#7d63e0] font-bold">Edit</button>
        ...
        <article className="panel p-5 space-y-3 rounded-2xl border border-[#E1D9F9]/[0.06]">
          <h2 className="font-heading text-2xl text-[#F6C545]">{editingHandbookId ? "✏️ Edit Handbook Title" : "📘 Handbook Upload Center"}</h2>
          <input value={newHandbookTitle} onChange={(e) => setNewHandbookTitle(e.target.value)} placeholder="Handbook Document Title" className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9] outline-none" />
          <div className="pt-2">
          ...
          <div className="mt-4 space-y-2 max-h-48 overflow-y-auto">
            {handbooks.map((hb) => (
              <div key={hb.id} className="p-3 bg-black/15 border border-[#E1D9F9]/10 rounded-xl flex justify-between items-center text-xs">
                <span className="text-[#E1D9F9] truncate max-w-[200px]">{hb.title}</span>
                <div className="flex gap-3">
                  <button type="button" onClick={() => { setEditingHandbookId(hb.id); setNewHandbookTitle(hb.title); }} className="text-[#7d63e0] font-bold">Edit</button>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <article className="panel p-5 space-y-4 rounded-2xl border border-[#E1D9F9]/[0.06]">
          ...
          <textarea value={newAnnouncementContent} onChange={(e) => setNewAnnouncementContent(e.target.value)} placeholder="Ketik isi body pengumuman pakai simbol di atas..." className="w-full rounded-xl border border-[#E1D9F9]/15 bg-[#0A0A0B]/50 px-3 py-2 text-xs text-[#E1D9F9] font-mono" rows={3} />
          ...
          <div className="flex gap-2">
            <input value={newLinkLabel} onChange={(e) => setNewLinkLabel(e.target.value)} placeholder="Label" className="w-1/3 bg-[#0A0A0B] rounded-lg p-2 text-xs text-[#E1D9F9] border border-[#E1D9F9]/10" />
            <input value={newLinkUrl} onChange={(e) => setNewLinkUrl(e.target.value)} placeholder="URL" className="w-2/3 bg-[#0A0A0B] rounded-lg p-2 text-xs text-[#E1D9F9] border border-[#E1D9F9]/10" />
            <button type="button" onClick={addLink} className="nav-chip text-xs">Attach</button>
          </div>
          
          {newAnnouncementLinks.length > 0 && (
            <div className="mt-2 p-3 bg-black/20 rounded-xl border border-[#E1D9F9]/5 space-y-2 animate-revealUp">
              ...
              {newAnnouncementLinks.map((link, index) => (
                <div key={index} className="flex justify-between items-center bg-[#0A0A0B] border border-[#E1D9F9]/10 p-2 rounded-lg text-[11px] text-[#E1D9F9]">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pt-2 border-t border-[#E1D9F9]/10">
            {announcements.map((a) => (
              <div key={a.id} className="p-3 bg-black/10 border border-[#E1D9F9]/5 rounded-xl flex justify-between items-center text-xs text-[#E1D9F9]">
                <span className="truncate max-w-[200px]">{a.title}</span>
                <div className="flex gap-3">
                  <button type="button" onClick={() => { setEditingAnnId(a.id); setNewAnnouncementTitle(a.title); setNewAnnouncementContent(a.content); setNewAnnouncementLinks(a.links || []); }} className="text-[#7d63e0] font-bold">Edit</button>
      ...
      <article className="panel p-5 rounded-2xl border border-[#E1D9F9]/[0.06]">
        <div className="flex justify-between items-center border-b border-[#E1D9F9]/10 pb-3 mb-3 flex-wrap gap-2">
          <h2 className="font-heading text-xl text-[#E1D9F9]">{t("Submissions & Grading")}</h2>
          <div className="flex gap-2 items-center">
            <button type="button" onClick={downloadSubmissionsCsv} className="nav-chip text-xs bg-[#452ABC]/30 text-[#E1D9F9] font-bold px-3 py-1.5 hover:bg-[#452ABC]/55 transition">+ Export Filtered CSV</button>
            
            <select value={submissionTaskFilter} onChange={(e) => setSubmissionTaskFilter(e.target.value)} className="bg-[#0A0A0B] text-xs text-[#E1D9F9] p-2 rounded-lg border border-[#E1D9F9]/10 cursor-pointer max-w-[150px] sm:max-w-[200px] truncate">
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto text-xs text-[#E1D9F9] pr-2">
          {filteredSubmissions.length > 0 ? (
            filteredSubmissions.map((sub) => (
              <div key={sub.id} className="p-3 bg-black/20 rounded-xl border border-[#E1D9F9]/5 flex justify-between items-center hover:bg-black/40 transition">
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0 pl-2">
                  ...
                  {sub.fileUrl && (
                    <a href={sub.fileUrl} target="_blank" rel="noreferrer" className="text-[10px] bg-[#452ABC]/30 px-3 py-1 rounded hover:bg-[#452ABC]/55 transition text-[#E1D9F9]">
            ...
            <p className="p-4 text-center text-[#E1D9F9]/50 italic bg-black/10 rounded-lg border border-[#E1D9F9]/5">
      ...
      <article className="panel p-5 space-y-4 rounded-2xl border border-[#E1D9F9]/[0.06]">
        <div className="flex justify-between items-center border-b border-[#E1D9F9]/10 pb-3 flex-wrap gap-2">
          <h2 className="font-heading text-xl text-[#7d63e0]">📊 Live Participant Attendance Submission Viewer</h2>
          <div className="flex gap-2 items-center">
            <button type="button" onClick={downloadAttendanceCsv} className="nav-chip text-xs bg-[#452ABC]/30 text-[#E1D9F9] font-bold px-3 py-1.5 hover:bg-[#452ABC]/55 transition">+ Export Filtered Attendance CSV</button>
            <select value={attendanceDayFilter} onChange={(e) => setAttendanceDayFilter(e.target.value)} className="bg-[#0A0A0B] text-xs text-[#E1D9F9] p-2 rounded-lg border border-[#E1D9F9]/10 cursor-pointer">
            ...
            <select value={attendanceTabFilter} onChange={(e) => setAttendanceTabFilter(e.target.value as "awal" | "akhir" | "h1")} className="bg-[#0A0A0B] text-xs text-[#E1D9F9] p-2 rounded-lg border border-[#E1D9F9]/10 cursor-pointer">
              <option value="awal">Check-In</option> <option value="akhir">Check-Out</option> <option value="h1">H-1 Medis</option>
            </select>
          </div>
        </div>
        <div className="overflow-x-auto rounded-xl border border-[#E1D9F9]/5 bg-black/10 text-xs text-[#E1D9F9] shadow-inner">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-[#E1D9F9]/5 text-[#F6C545] border-b border-[#E1D9F9]/10 whitespace-nowrap">
            ...
            <tbody>
              {adminAttendanceRecords.length > 0 ? (
                adminAttendanceRecords.map((row, idx) => (
                  <tr key={idx} className="border-b border-[#E1D9F9]/5 hover:bg-[#E1D9F9]/5 transition">
                    <td className="p-3 font-mono text-[#7d63e0] font-bold whitespace-nowrap">{row.nim}</td>
                    <td className="p-3 font-medium min-w-[150px]">{row.fullName}</td>
                    <td className="p-3 whitespace-nowrap"><span className="px-2 py-0.5 rounded bg-[#E1D9F9]/10 text-[10px] uppercase font-bold">{row.status}</span></td>
                ...
                <tr><td colSpan={6} className="p-4 text-center text-[#E1D9F9]/50 italic">Belum ada sirkulasi paket log absensi maba masuk pada parameter filter ini.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function PromptEditor({ id, text, order, onSave, onDelete }: { id: string; text: string; order: number; onSave: (id: string, text: string, order: number) => Promise<void>; onDelete: (id: string) => Promise<void>; }) {
  ...
  return (
    <div className="rounded-xl border border-[#E1D9F9]/10 bg-black/15 p-2 flex gap-2 items-center animate-revealUp">
      <input value={draftText} onChange={(e) => setDraftText(e.target.value)} className="w-full bg-[#0A0A0B]/50 border border-[#E1D9F9]/10 rounded px-2 py-1 text-xs text-[#E1D9F9] outline-none" />
