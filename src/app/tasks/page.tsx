  ...
  return (
    <section className="space-y-4">
      <header className="panel p-6 sm:p-8">
        ...
        <h1 className="mt-3 font-heading text-5xl tracking-wider text-[#E1D9F9]">{t("Task Info & Submission")}</h1>
        <p className="mt-3 max-w-2xl text-[#E1D9F9]/80">{t("Check active assignments, read the brief, and submit your work in one place.")}</p>
      </header>

      {isTaskGateClosed && (
        <div className="panel p-4 border border-[#EC5C2A]/40 bg-[#EC5C2A]/10 text-center rounded-xl animate-revealUp">
          <p className="text-sm font-bold text-[#EC5C2A] uppercase tracking-wider">🔒 MISSION CLOSED - Batas Pengumpulan Habis</p>
          <p className="text-xs text-[#E1D9F9]/50 mt-1">Gerbang pengumpulan berkas otomatis dikunci sistem karena melewati batas waktu panitia pusat.</p>
        </div>
      )}

      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <article className="panel p-5">
          <h2 className="font-heading text-3xl tracking-wider text-[#E1D9F9]">{t("Current Tasks")}</h2>
              ...
              <button
                ...
                className={`w-full rounded-xl border p-4 text-left transition ${
                  selectedTask === task.taskId
                    ? "border-[#F6C545] bg-[#F6C545]/15"
                    : "border-white/15 bg-white/5 hover:border-white/30"
                }`}
              >
                <p className="text-xs uppercase tracking-[0.08em] text-[#E1D9F9]/50">{task.taskId}</p>
                <h3 className="mt-1 text-xl text-[#E1D9F9]">{task.title}</h3>
                <p className="mt-1 text-sm text-[#F6C545]">{t("Deadline: ")}{task.deadline}</p>
              </button>
            ))}
          </div>
        </article>
        ...
        <article className="panel p-5">
          <h2 className="font-heading text-3xl tracking-wider text-[#E1D9F9]">{t("Task Brief")}</h2>
          {activeTask ? (
            <>
              <p className="mt-3 text-[#E1D9F9]/80 whitespace-pre-wrap">{activeTask.detail}</p>

              {(activeTask.handbookUrl || activeTask.taskFileUrl) && (
                <div className="mt-4 space-y-3 border-t border-white/15 pt-4">
                  <p className="text-sm font-medium text-[#F6C545]">{t("Task Resources")}</p>
              ...
              <div className="mt-4 rounded-xl border border-white/15 bg-black/15 p-4 text-sm text-[#E1D9F9]/80">
                <p className="text-[#F6C545]">{t("Submission rules")}</p>
            ...
            <p className="mt-3 text-[#E1D9F9]/50">{t("No tasks available. Check back later!")}</p>
          )}
        </article>
      </div>
      ...
      <article className="panel p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="font-heading text-3xl tracking-wider text-[#E1D9F9]">{t("Submit Your Work")}</h2>
          <button type="button" onClick={exportCsv} className="nav-chip px-4 py-2">{t("Export CSV")}</button>
        </div>

        <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
          <label className="block space-y-1">
            <span className="text-sm text-[#E1D9F9]/80">NIM Anda (Identitas Terkunci)</span>
            <input type="text" value={studentNIM} disabled className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[#E1D9F9] font-mono font-bold opacity-50 outline-none" />
          </label>

          <label className="block space-y-1">
            <span className="text-sm text-[#E1D9F9]/80">Pilih Format Media Pengumpulan</span>
            <select
              ...
              className="notranslate w-full rounded-lg border border-white/25 bg-black/20 px-3 py-2 text-[#E1D9F9] outline-none cursor-pointer disabled:opacity-50"
            >
          ...
          {effectiveSubmissionType === "link" ? (
            <label className="block space-y-1 animate-revealUp">
              <span className="text-sm text-[#F6C545]">Masukkan Link URL Pengerjaan Tugas (Wajib Diisi)</span>
              <input
                type="url"
                ...
                className="w-full rounded-lg border border-white/25 bg-black/20 px-3 py-2 text-white outline-none focus:border-[#F6C545]"
              />
            </label>
          ) : (
            <label className="block space-y-1 animate-revealUp">
              <span className="text-sm text-[#F6C545]">Unggah Lampiran Berkas File Tugas (Wajib Diisi)</span>
              <input
                type="file"
                ...
                disabled={isTaskGateClosed}
                className="w-full rounded-lg border border-white/25 bg-black/20 px-3 py-2 text-[#E1D9F9]/80 cursor-pointer file:rounded file:border-0 file:bg-[#452ABC] file:text-white file:px-2 file:py-0.5 file:text-xs"
              />
            </label>
          )}

          <label className="block space-y-1">
            <span className="text-sm text-[#F6C545]">{t("Short Note")}</span>
            <textarea
              value={note}
              ...
              className="w-full rounded-lg border border-white/25 bg-black/20 px-3 py-2 text-[#E1D9F9] outline-none"
            />
          </label>
        ...
        {saveMessage && (
          <div className={`mt-4 p-3 rounded-lg border font-bold text-sm tracking-wide animate-revealDown ${saveMessage.startsWith("✅") ? 'bg-[#452ABC]/30 border-[#452ABC] text-[#E1D9F9]' : saveMessage.startsWith("❌") ? 'bg-[#EC5C2A]/20 border-[#EC5C2A] text-[#EC5C2A]' : 'bg-[#F6C545]/12 border-[#F6C545]/40 text-[#E1D9F9]'}`}>
            {saveMessage}
          </div>
        )}
      </article>
